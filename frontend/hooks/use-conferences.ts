...
import type { ConferenceCreatePayload, ConferenceUpdatePayload } from '@/types/conference';
...
export function useCreateConference() {
  ...
  return useMutation({
    mutationFn: (payload: ConferenceCreatePayload) => conferenceService.create(payload),
...
export function useUpdateConference() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: ({ id, payload }: { id: string; payload: ConferenceUpdatePayload }) =>
